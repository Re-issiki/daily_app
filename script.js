...
function createQuestElement(obj, category, index) {
    ...
    checkbox.addEventListener("change", () => {
        ...
        // 経験値加算（レア度による）
        catData.exp += obj.rarity === "bronze" ? 10 :
                        obj.rarity === "silver" ? 20 : 30;
        ...
        // 現ランクまでの必要EXPを計算
        for (let i = 0; i < rankOrder.indexOf(currentRank); i++)
            expPerRank *= 2;
    ...
    radarChart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                data: values,
                borderWidth: 2,
                backgroundColor: "rgba(33, 150, 243, 0.4)", // 青
                borderColor: "rgb(33, 150, 243)",          // 青
                pointBackgroundColor: "rgb(33, 150, 243)"  // 青
            }]
        },
        options: {
            scales: {
...
function getExpForRank(rank) {
    ...
    for (let i = 0; i < rankOrder.indexOf(rank); i++)
        exp *= 2;
    return exp;
}
...
function rankToNumber(rank) {
    ...
    return order.indexOf(rank) + 1; // F=1 〜
}
